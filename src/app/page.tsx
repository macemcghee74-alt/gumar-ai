  ...
  return (
    <main className="shell">
      ...
      <section className="workspace">
        <header><div><p className="eyebrow">{showMemories ? "PRIVATE MEMORY" : "PRIVATE SESSION"}</p><h1>{showMemories ? "Memory Center." : "Stay curious."}</h1></div><span className="badge">GUNMAR / ALPHA</span></header>
        ...
        </section> : <div className="transcript">
        ...
        {!showMemories && <form className="composer" onSubmit={sendMessage}><textarea value={input} onChange={(event) => setInput(event.target.value)} placeholder="Say something to Gunmar…" rows={2} disabled={busy} /><button type="submit" disabled={busy || !input.trim()}>{busy ? "…" : "Send"} <span>↗</span></button></form>}

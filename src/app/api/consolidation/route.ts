...
export async function POST() {
  ...
  if (memoryError) return NextResponse.json({ error: "Unable to load memories for consolidation." }, { status: 503 });
  ...
  for (const group of duplicateGroups.values()) {
    ...
    for (const duplicate of duplicates) {
      ...
      if (error) return NextResponse.json({ error: "Unable to consolidate duplicate memories." }, { status: 503 });
      superseded += 1;
    }
  }
  ...
  if ((count ?? 0) >= 3) await evolvePersonality(supabase, user.id, "reflectiveness", { value: 0.55, reason: "Repeated interaction history supports a small bounded adjustment." });
  ...
  if (auditError) return NextResponse.json({ error: "Unable to record consolidation audit." }, { status: 503 });
